...
interface ProjectCardProps extends ProjectDetails {
    onClick: () => void;
}

export const ProjectCard = ({ title, description, imageSrc, techStack, liveUrl, githubUrl, isLatest, onClick }: ProjectCardProps) => {
    return (
        <div 
            className="group flex flex-col bg-[#1c334a]/50 border border-white/5 rounded-2xl overflow-hidden hover:bg-[#2c334a] transition-all duration-300 shadow-lg hover:shadow-blue-900/20 relative cursor-pointer"
            onClick={onClick}
        >
            {isLatest && (
            ...
            <div className="flex flex-col p-6 gap-4 grow">
                ...
                <div className="flex gap-4 mt-4 pt-4 border-t border-white/5">
                    {liveUrl && (
                        <a href={liveUrl} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="text-sm font-krona text-blue-400 hover:text-blue-300 transition-colors">
                            Live Demo ↗
                        </a>
                    )}
                    {githubUrl && (
                        <a href={githubUrl} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="text-sm font-krona text-gray-400 hover:text-white transition-colors">
                            Source Code ↗
                        </a>
                    )}
                </div>
